# Добавьте обработку клавиш PgUp
# и PgDown, по нажатию на которые необходимо увеличивать и уменьшать масштаб отображения карты.
...
class App(QWidget):
    ...
    def findf(self, inc=False, dec=False):
        coords = self.cordsEdit.text().replace(" ", "").split(",")
        if not inc and not dec:
            self.zoom = int(self.zoomBox.text())
        self.zoom += 1 if inc else -1
        if self.zoom == 22:
            self.zoom -= 1
        elif self.zoom == -1:
            self.zoom += 1
        res = requests.get(f"https://static-maps.yandex.ru/1.x/?ll={coords[1]},{coords[0]}&l=map&z={str(self.zoom)}")
        if res:
